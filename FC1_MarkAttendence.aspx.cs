...
public partial class MarkAttendence : System.Web.UI.Page
{
    ...
    private List<SectionInfo> getSectionList()
    {
        ...
        string query = "SELECT 0 AS ID, '-' AS Name UNION SELECT SECTION.Section_Id AS ID, SECTION.Name AS Name FROM SECTION INNER JOIN OFFEREDCOURSE ON SECTION.Course_Id = OFFEREDCOURSE.OfferCourse_Id " +
    ...
    private void LoadStudentSheet(string class_Id, string date)
    {
        ...
        string query = "SELECT USERS.User_Id AS ID, RegNo, CONCAT(First_Name, ' ', Last_Name) AS Student_Name, Status FROM USERS INNER JOIN USERACCOUNT " +
        ...
        while (reader.Read())
        {
            ...
            std.Status = Convert.ToChar(reader.GetValue(3));
            Students.Add(std);
        }
        SectionStudentsList.DataSource = Students;
        SectionStudentsList.DataBind();
    }
    protected void SectionStudentsList_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            CheckBox checkBox = (CheckBox)e.Row.FindControl("chkEnabled");

            if (Students[e.Row.RowIndex].Status == 'P')
    ...
    {
        if (SectionList.SelectedIndex == 0) return;
    ...
    private string GeneratePresentStudent(string classId, string section)
    {
        bool UpdatesMade = false;
        string res = "UPDATE ATTENDENCE SET Status = 'P' WHERE Student_Id IN (";
        ...
        res = res.Remove(res.Length - 1, 1);
        res += ") AND Class_Id = " + classId;
        if (!UpdatesMade) res = "";
        return res;
    }
    private string GenerateAbsentStudent(string classId, string section)
    {
        bool UpdatesMade = false;
    ...
    protected void SecSaveButton_Click(object sender, EventArgs e)
    {
        ...
        string query = GenerateAbsentStudent(class_id, section);
        if (query != "")
        {
            ExecuteNonQuery(query);
            query = GeneratePresentStudent(class_id, section);
            ExecuteNonQuery(query);
