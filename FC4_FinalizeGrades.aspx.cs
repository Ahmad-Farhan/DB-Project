...
public partial class FC4_FinalizeGrades : System.Web.UI.Page
{
    ...
    private string Numify(string val)
    {
        if (val == "")
            return "0";
        else return val;
    ...
    private List<Std> getStudentMarksList(string section_Id)
    {
        ...
        string query = @"SELECT T.Student_Id, SUM(AbsoluteScored) FROM MARKS M
        ...
        while (reader.Read())
        {
            ...
            std.Obtained = Numify(reader.GetValue(1).ToString());
    ...
    private void UpdateSectionGrades(string section_Id)
    {
        ...
        ExecuteNonQuery(query);
        LogEvent("Finalized Grades for section " + section_Id);
    }
