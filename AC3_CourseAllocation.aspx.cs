...
public partial class CourseAllocation : System.Web.UI.Page
{
    ...
    public void SectionInsert()
    {
        ...
        LogEvent(Convert.ToInt32(User_Id), "Created Sections : " + SectionsAdded.ToString());
    }
    public void GetStudentsList(string crsId, List<string> studentIds)
    {
        string query = "SELECT Student_Id FROM REGISTERATION WHERE Course_Id = " + crsId + " AND Semester = '" + semester + "'";
        ...
        while(reader.Read())
            studentIds.Add(reader.GetValue(0).ToString());

        con.Close();
    }
    public string ConstructTranscriptIns(List<string> students, int prev, string sectionId)
    {
        string res = "INSERT INTO TRANSCRIPT (Student_Id, Section_Id) VALUES \n";
        for (int i = prev; i - prev < MaxLimit && i < students.Count; i++)
            res += "(" + students[i] + "," + sectionId + "),\n";
        res = res.Remove(res.Length - 2, 2);
        return res;
    }
    public void GetSectionList(string crsId, List<string> sectionslist)
    {
        string query = "SELECT Section_Id FROM SECTION WHERE Course_Id = " + crsId ;
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["FlexConnectionString"].ConnectionString);
        con.Open();
        SqlCommand cm = new SqlCommand(query, con);
        SqlDataReader reader = cm.ExecuteReader();
        ...
        while (reader.Read())
            sectionslist.Add(reader.GetValue(0).ToString());

        con.Close();
    }
    public void AssignStudentSections()
    {
        string Insquery;
        ...
        {
            GetStudentsList(sections[i].Course_Id, student_Ids);
            GetSectionList(sections[i].Course_Id, section_Ids);
            for (int j = 0, prev = 0; j < section_Ids.Count; j++)
            {
                Insquery = ConstructTranscriptIns(student_Ids, prev, section_Ids[j]);
                ExecuteNonQuery(Insquery);
                prev += MaxLimit;
            }
        }
    }
    public bool SectionCreationAvailable()
    {
        string res = ExecuteScalar("SELECT Section_Id FROM SECTION S INNER JOIN OFFEREDCOURSE O ON O.OfferCourse_Id = S.Course_Id WHERE Section_Id IS NULL", "Section_Id");
        return (res != "");
    }
    public void CreateSections(string semester)
    {
        ...
        GetNoOfSectionsPerCourse();
        SectionInsert();
        AssignStudentSections();
        SuccessLabel0.Visible = true;
        TriggerSectionCreation.Visible = false;
        Page_Load(null, null);
    }
    public void GetNoOfSectionsPerCourse()
    {
    ...
    public void GetSections(string semester)
    {
        string query = "SELECT Course_Id, COUNT(Course_Id) AS TotalRegistered FROM REGISTERATION WHERE Semester = '" + semester +
            "' GROUP BY Course_Id UNION SELECT O.OfferCourse_Id, 0 AS TotalRegistered FROM OFFEREDCOURSE O LEFT JOIN REGISTERATION R ON" +
            " O.OfferCourse_Id = R.Course_Id WHERE OfferedIn = '" + semester + "' AND R.Reg_Id IS NULL";
    ...
    {
        string query = "DELETE FROM OFFEREDCOURSE WHERE OfferCourse_Id = " + Course_Id;
        ...
        ExecuteNonQuery("DELETE FROM REGISTERATION WHERE Course_Id = " + Course_Id);
    }
