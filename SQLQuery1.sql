...
delete from section
select * from section
select * from registeration WHERE Course_Id = 7

select * from offeredcourse where course_Id = 6

SELECT 0 AS ID, '-' AS Name UNION SELECT SECTION.Section_Id AS ID, SECTION.Name AS Name FROM SECTION INNER JOIN OFFEREDCOURSE ON SECTION.Course_Id = OFFEREDCOURSE.OfferCourse_Id INNER JOIN COURSE ON COURSE.Course_Id = OFFEREDCOURSE.Course_Id WHERE Instructor_Id = 7 AND OfferedIn = 'Spring 2023' AND Course_Code = 'CS1002'
select * from useraccount where User_id = 9

SELECT Name, Weightage, Range, Obtained FROM EVALUATION INNER JOIN MARKS ON EVALUATION.Eval_Id = MARKS.Eval_Id AND Student_Id = 9

select * from marks

SELECT Section_Id FROM SECTION S INNER JOIN OFFEREDCOURSE O ON O.OfferCourse_Id = S.Course_Id WHERE Section_Id IS NULL
Select Course_Id from Registeration where Semester = 'Spring 2023' AND 
Course_Id NOT IN (S);

SELECT O.Course_Id, COUNT(O.Course_Id) TotalRegistered FROM REGISTERATION 
UNION SELECT Course_Id, 0 FROM OFFEREDCOURSE WHERE OfferedIn = 'Spring 2023'

SELECT Course_Id, COUNT(Course_Id) AS TotalRegistered FROM REGISTERATION WHERE Semester = 'Spring 2023' GROUP BY Course_Id
UNION SELECT O.OfferCourse_Id, 0 AS TotalRegistered FROM OFFEREDCOURSE O LEFT JOIN REGISTERATION R ON O.OfferCourse_Id = R.Course_Id
WHERE OfferedIn = 'Spring 2023' AND R.Reg_Id IS NULL

SELECT * FROM OFFEREDCOURSE O LEFT JOIN REGISTERATION R ON O.OfferCourse_Id = R.Course_Id
WHERE OfferedIn = 'Spring 2023' AND R.Reg_Id IS NULL


SELECT R.Course_Id FROM REGISTERATION R RIGHT JOIN OFFEREDCOURSE O ON R.Course_Id = O.OfferCourse_Id

SELECT T.Student_Id, SUM(AbsoluteScored) FROM MARKS M
RIGHT JOIN TRANSCRIPT T ON T.Student_Id = M.Student_id
WHERE T.Section_Id = 34 GROUP BY T.Student_Id, T.Section_Id

SELECT '-' AS Name, 0 as Course_id UNION SELECT C.Name, S.Course_Id FROM SECTION S INNER JOIN OFFEREDCOURSE O ON 
O.OfferCourse_Id = S.Course_Id INNER JOIN COURSE C ON C.Course_Id = O.Course_Id WHERE Instructor_Id = 2

SELECT (SUM(Eval1) * 100 / (COUNT(Eval1) * 5)) AS Eval1_Percent,
(SUM(Eval2) * 100 / (COUNT(Eval2) * 5)) AS Eval2_Percent,
(SUM(Eval3) * 100 / (COUNT(Eval3) * 5)) AS Eval3_Percent,
(SUM(Eval4) * 100 / (COUNT(Eval4) * 5)) AS Eval4_Percent,
(SUM(Eval5) * 100 / (COUNT(Eval5) * 5)) AS Eval5_Percent
FROM FEEDBACK WHERE Instructor_Id = 2 GROUP BY Instructor_Id

SELECT * FROM USERACCOUNT UA INNER JOIN ATTENDENCE A ON UA.User_Id = A.Student_Id


SELECT User_Id, 
CASE WHEN Status = 'P' THEN 1 END, 




Select '-' as Name, 0 as Course_Id UNION Select COURSE.Name, Section.Course_Id  from SECTION inner join OFFEREDCOURSE
on COURSE on COURSE.Course_Id = SECTION.Course_Id where Instructor_Id = @instructorId";

select * from feedback


SELECT T.Student_Id FROM TRANSCRIPT T
WHERE T.Section_Id = 34

select Student_Id from registeration where course_id = 1
SELECT Section_Id FROM SECTION WHERE Course_Id = 1

SELECT Student_Id FROM REGISTERATION WHERE Course_Id = 1

select * from transcript where Student_Id = 10

SELECT Grade, COUNT(Grade) AS Total_Grades FROM TRANSCRIPT T INNER JOIN SECTION S ON S.Section_Id = T.Section_Id
INNER JOIN OFFEREDCOURSE O ON O.OfferCourse_Id = S.Course_Id WHERE S.Course_Id = 1 GROUP BY Grade

, O.Course_Id HAVING O.Course_Id = 1

select Grade ,count(Grade) as Total_Grades from TRANSCRIPT inner join Section on TRANSCRIPT.Section_Id=SECTION.Section_Id
inner join OFFEREDCOURSE on OFFEREDCOURSE.OfferCourse_Id =Section.Course_Id group by Grade having OFFEREDCOURSE.Course_Id=1

SELECT USERS.User_Id AS ID, RegNo, CONCAT(First_Name, ' ', Last_Name) AS Student_Name FROM TRANSCRIPT INNER JOIN USERACCOUNT ON TRANSCRIPT.Student_Id = USERACCOUNT.User_Id INNER JOIN USERS ON USERS.User_Id = TRANSCRIPT.Student_Id WHERE TRANSCRIPT.Section_Id = 1
select Section_Id FROM section inner join offeredcourse on section.Course_Id = offeredcourse.Course_Id
where section_Id IS NUll

SELECT Section_Id FROM SECTION S INNER JOIN OFFEREDCOURSE O ON O.OfferCourse_Id = S.Course_Id
WHERE Section_Id IS NULL
...
select * from transcript where Student_Id = 9
select * from class
select * from attendence where Student_Id = 10

select * from Users where User_id = 9


select * from registeration r inner join offeredcourse o on 
r.Course_Id = o.OfferCourse_Id

SELECT Course_Code, COURSE.Name, TRANSCRIPT.Section_Id FROM COURSE INNER JOIN OFFEREDCOURSE ON 
            ...
            TRANSCRIPT ON TRANSCRIPT.Section_Id = SECTION.Section_id WHERE Student_Id = 9 AND OfferedIn = 'Spring 2023'

SELECT Course_Code, C.Name, T.Section_Id FROM COURSE C INNER JOIN OFFEREDCOURSE O ON O.Course_Id = C.Course_Id
INNER JOIN SECTION S ON S.Course_Id = O.OfferCourse_Id INNER JOIN TRANSCRIPT T
ON T.Section_Id = S.Section_Id WHERE T.Student_Id = 9 AND O.OfferedIn = 'Spring 2023'
